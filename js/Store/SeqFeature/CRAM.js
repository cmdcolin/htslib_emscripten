define( [
        ...
        function( declare, array, has, Util, Errors, LRUCache, CRAMUtil, CRAMFeature ) {

var CRAM_MAGIC = 21840194;
...
var CramFile = declare( null, {
    ...
    _readCRAI: function( successCallback, failCallback ) {
        ...
        this.crai.fetch( dojo.hitch( this, function(header) {
            ...
            for (var ref = 0; ref < nref; ++ref) {
                ...
                // tells us where the CRAM header ends
    ...
    _readCRAMheader: function( successCallback, failCallback ) {
        var thisB = this;
        ...
        thisB.data.read(
            ...
            function(r) {
                try {
                    var uncba;
                    try {
                        uncba = new Uint8Array( CRAMUtil.unbgzf(r) );
                    } catch(e) {
                        throw new Error( "Could not uncompress CRAM data. Is it compressed correctly?" );
                    }

                    if( readInt(uncba, 0) != CRAM_MAGIC)
                        throw new Error('Not a CRAM file');
        ...
        thisB.data.read( 0, start+refSeqBytes,
            ...
            for (var i = 0; i < nRef; ++i) {
                ...
                if( p > uncba.length ) {
                    ...
                    refSeqBytes *= 2;
                    console.warn( 'CRAM header is very big.  Re-fetching '+refSeqBytes+' bytes.' );
    ...
    _fetchChunkFeatures: function( chunks, chrId, min, max, featCallback, endCallback, errorCallback ) {
        ...
        for( var i = 0; i<chunks.length; i++ ) {
            ...
            if( size > this.chunkSizeLimit ) {
                errorCallback( new Errors.DataOverflow('Too many CRAM features. CRAM chunk size '+Util.commifyNumber(size)+' bytes exceeds chunkSizeLimit of '+Util.commifyNumber(this.chunkSizeLimit)+'.' ) );
                return;
            }
        }
    ...
    _readChunk: function( chunk, callback ) {
        ...
        thisB.data.read( chunk.minv.block, chunk.fetchedSize(), function(r) {
            try {
                var data = CRAMUtil.unbgzf(r, chunk.maxv.block - chunk.minv.block + 1);
                thisB.readCramFeatures( new Uint8Array(data), chunk.minv.offset, features, callback );
            } catch( e ) {
    ...
    readCramFeatures: function(ba, blockStart, sink, callback ) {
        var that = this;
        var featureCount = 0;
        ...
        while ( true ) {
            if( blockStart >= ba.length ) {
                ...
                window.setTimeout( function() {
                    that.readCramFeatures( ba, blockStart, sink, callback );
                }, 1);
                return;
            }
        }
